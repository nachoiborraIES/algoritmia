...
* Para leer cada palabra por separado de las siguientes puede usar el método `next()` de `Scanner` (`sc.next()`), ue va procesando cada *token* o palabra clave que encuentra.
